...
int main(int argc, char** argv) {
	...
	if(config.fullscreen){
		...
		window = glfwCreateWindow(mode->width, mode->height, "PBRDemo", glfwGetPrimaryMonitor(), NULL);
	} else {
		// Create a window with a given size. Width and height are defined in the configuration.
		window = glfwCreateWindow(config.initialWidth, config.initialHeight,"PBRDemo", NULL, NULL);
	}
