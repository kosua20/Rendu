...
FloodFill::FloodFill(unsigned int width, unsigned int height) {
...
void FloodFill::process(const GLuint textureId) {
...
void FloodFill::resize(unsigned int width, unsigned int height){
	_iterations = std::ceil(std::log2(std::max(width, height)));
