...
layout(set = 2, binding = 8) uniform texture2D iridescenceTexture; ///< Iridescence IOR and thickness.
...
void main(){
	...
	material.id = MATERIAL_IRIDESCENT; // Transparent surfaces behave as a standard PBR material.
	...
	vec2 decodedIridescence = decodeIridescence(iridescenceInfo.x, iridescenceInfo.y);
	...
	for(int pid = 0; pid < MAX_PROBES_COUNT; ++pid){
		...
		// Apply transparency on the diffuse.
		fragColor += weight * vec4(albedoInfos.a * diffuse + specular, 1.0);
	}
