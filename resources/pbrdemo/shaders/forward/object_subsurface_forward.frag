...
layout(set = 2, binding = 7) uniform texture2D effectsTexture; ///< Effects map (thickness in A channel).
layout(set = 2, binding = 8) uniform texture2D subsurfaceTexture; ///< Subsurface color and roughness.
...
void main(){
	...
	material.id = MATERIAL_SUBSURFACE;
	...
	vec4 infos = texture(sampler2D(effectsTexture, sRepeatLinearLinear), In.uv);
	material.roughness = max(0.045, infos.r);
	material.ao = infos.b;
	// Conductors can't have internal scattering and transmission.
