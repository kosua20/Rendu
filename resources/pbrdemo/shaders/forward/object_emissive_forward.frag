...
layout(set = 2, binding = 0) uniform texture2D emissiveTexture; ///< Emissive texture.
...
void main(){
	...
	vec4 emissiveColor = texture(sampler2D(emissiveTexture, sRepeatLinearLinear), In.uv);
	if(emissiveColor.a <= 0.01){
		discard;
	}

	Material material = initMaterial();
	material.id = MATERIAL_EMISSIVE;
	...
	float roughness = texture(sampler2D(effectsTexture, sRepeatLinearLinear), In.uv).r;
	material.roughness = max(0.045, roughness);
	material.ao = 1.0;
