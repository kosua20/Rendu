...
layout(set = 2, binding = 3) uniform texture2D texture3; ///< Iridescence IOR and thickness.
...
void main(){
	...
	vec2 iridescenceInfo = texture(sampler2D(texture3, sRepeatLinearLinear), In.uv.xy).rg;

	// Store values.
	fragColor.rgb = color.rgb;
	fragColor.a = encodeMaterial(MATERIAL_IRIDESCENT);
	fragNormal.rg = encodeNormal(n);
	// Iridescence IOR.
	fragNormal.b = iridescenceInfo.r;
	fragNormal.a = 0.0;
	...
	fragEffects.a = iridescenceInfo.g;
}
