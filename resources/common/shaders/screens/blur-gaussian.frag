...
void main(){
	...
	vec3 col = textureLod(sampler2D(screenTexture, sClampLinear), In.uv, 0.0).rgb * 6.0/16.0;
	col += textureLod(sampler2D(screenTexture, sClampLinear), In.uv - fetchOffset, 0.0).rgb * 5.0/16.0;
	col += textureLod(sampler2D(screenTexture, sClampLinear), In.uv + fetchOffset, 0.0).rgb * 5.0/16.0;
	fragColor = col;
}
