...
vec3 shading(vec2 uv, vec3 n, vec3 v,vec3 lightPosition, float lightShininess, vec3 lightColor, float specularOcclusion, out vec3 ambient){
...
void main(){
	...
	if(materialId == 2){
		...
		// Query UV offset.
		localUV = parallax(In.uv, vTangentDir);
	...
	if(materialId == 2){
		...
		float shadowParallax = parallaxShadow(localUV, lTangentDir);
